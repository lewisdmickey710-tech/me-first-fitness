-- Pricing decisions finalized after 0071 shipped: video sessions are $20
-- (not $25) and run 60 minutes -- same length as a check-in call, so "all
-- bookings" are 60 minutes now. Fully virtual (async) programming gets a
-- real fixed rate, $90/month, replacing the placeholder "billed as agreed"
-- language from 0071.

drop policy if exists "payments: client creates own video session balance" on public.payments;
...
- Video sessions are an optional add-on available to either in-person or virtual clients, booked and paid for individually through the app -- 60 minutes each
- Standalone written program available without sessions ($50)
- One-time 60-minute check-in call, available to any client regardless of session mode

## 2. Rates & Payment

- In-Person Session: $40 / session
- Virtual (fully async programming): $90 / month
- Video Session (add-on, either plan): $20 / session (60 minutes), paid via Cash App or Zelle at the time of booking -- your coach confirms the timeslot once payment is received
- Standalone Written Plan: $50 (no sessions)
- Check-in calls (60 minutes) are included as a courtesy, available any time regardless of plan
